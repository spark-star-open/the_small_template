...
function buildColumnsFromList(list, columns){
  ...
  // 动态编号
  const doorNums=new Set(), winNums=new Set(), partNums=new Set(), glassNums=new Set(), lsNums=new Set()
  ;(list||[]).forEach(r=>{ Object.keys(r||{}).forEach(k=>{ let m=k.match(/^door(\d+)/); if(m) doorNums.add(+m[1]); m=k.match(/^window(\d+)/); if(m) winNums.add(+m[1]); m=k.match(/^partition(\d+)/); if(m) partNums.add(+m[1]); m=k.match(/^glass(\d+)/); if(m) glassNums.add(+m[1]); m=k.match(/^lightSteel(\d+)/); if(m) lsNums.add(+m[1]); }) })
  ...
  // 其余字段
  const unionKeys = Array.from((list||[]).reduce((s,r)=>{ Object.keys(r||{}).forEach(k=>{ if(!ignore.has(k)) s.add(k) }); return s }, new Set())).sort()
  unionKeys.forEach(k=>push(cols,k))
  return cols
}
...
exports.main = async (event = {}) => {
  ...
  try {
    ...
    const rows = [header]
    list.forEach(r => rows.push(__cols.map(c => cellValue(r[c.key || c], c.key || c))))
